...
class Block:
    ...
    def set_rectangle(self, position, width, height):
        ...
        return pygame.Rect(position.x - (width / 2), position.y - (height / 2),
                           width, height)
...
class PulsingBlock(Block):
    ...
    def __init__(self, pulse_size, pulse_rate, bounds, position, width, height,
                 color):
    ...
    def update(self):
        ...
        self.rectangle = self.set_rectangle(self.position, self.width,
                                            self.height)
        if self.width >= self.base_width + self.pulse_size or self.width <= self.base_width:
            self.pulse_rate *= -1
        super().update()
