...
class KineticBall(Ball):
    ...
    def __init__(self, mass, object_list, bounds, position, velocity, color,
                 radius):
    ...
    def collide(self, object, relative_vector):
        ...
        angle2 = 0.5 * math.pi - math.atan2(object.velocity.y,
                                            object.velocity.x)
        angle1 = 2 * tangent - angle1
        angle2 = 2 * tangent - angle2
        object_speed = object.velocity.length()
        self_speed = self.velocity.length()
        self.velocity = Vector2(math.sin(angle1),
                                math.cos(angle1)) * object_speed
        object.velocity = Vector2(math.sin(angle2),
                                  math.cos(angle2)) * self_speed
